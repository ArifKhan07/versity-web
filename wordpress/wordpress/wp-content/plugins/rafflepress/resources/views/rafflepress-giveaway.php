<?php
    ...
    // if (current_user_can('manage_options') && empty($giveaway->active)) {
    ...
    $ref_url = home_url()."?rafflepress_id=".$giveaway->id;
    ...
    if (!empty($settings->entry_options)) {
        foreach ($settings->entry_options as $v) {
            ...
            if ($v->type== 'instagram-page-post' && !empty($v->post_url)) {
                $v->post_url = str_replace('\\',"",$v->post_url);
                $v->embed_code = wp_oembed_get($v->post_url );
                if ($v->embed_code) {
                    $include_instagram_sdk = true;
                    $v->embed_code = str_replace('<script async src="//www.instagram.com/embed.js"></script>', '', $v->embed_code);
                }
            }
...
try {
    ...
    <script data-cfasync="false"
        src="<?php echo RAFFLEPRESS_PLUGIN_URL; ?>public/js/iframeResizer.min.js?ver=<?php echo RAFFLEPRESS_VERSION; ?>">
    </script>
