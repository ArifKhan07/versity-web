...
class rafflepress_lite_Review {
	...
	public function review() {
        ...
        // Only proceed if they have created at least 1 giveaway that is atleast 7 days old
        global $wpdb;
        $tablename = $wpdb->prefix . 'rafflepress_giveaways';
        $sql = "SELECT id FROM $tablename WHERE deleted_at IS NULL and created_at <= (DATE(NOW()) - INTERVAL 2 DAY) ORDER BY created_at LIMIT 1";
        $number_of_giveaways = $wpdb->get_var($sql);
		if ( empty( $number_of_giveaways ) ) {
			return;
		}
