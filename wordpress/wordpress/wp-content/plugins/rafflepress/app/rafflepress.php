...
function rafflepress_lite_giveaway_api()
{
    ...
    if ($method == 'return') {
            ...
            foreach($entry_options as $k=>$v){
                if(!empty($v->type) && $v->type == 'join-newsletter'){
                    if(!empty($v->auto_trigger)){
                        global $rafflepress_queue_email;
                        $rafflepress_queue_email->data(array(
                            'contestant_id'=>$contestant['id'],
                            'action_id'=>$v->id,
        ...
        }else if($_FILES['file']['size'] > 5242880) { //5 MB (size is also in bytes)
...
?>
<script data-cfasync="false" src="<?php echo RAFFLEPRESS_PLUGIN_URL; ?>public/js/iframeResizer.min.js"></script>
...
<div class="rafflepress-giveaway-iframe-wrapper rafflepress_iframe_loading">
    <iframe id="rafflepress-<?php echo $id ?>"
        src="<?php echo home_url().'?rafflepress_page=rafflepress_render&rafflepress_id='.$id.'&iframe=1&giframe='.$a['giframe'].'&rpr='.$ref.'&parent_url='.urlencode($parent_url) ?>&<?php echo mt_rand(1, 99999); ?>"
        frameborder="0" scrolling="no" allowtransparency="true" <?php echo $style; ?> onload="rafflepress_resize_iframe_<?php echo $a['id'] ?>(this)"></iframe>
</div>
<script>
try{
iFrameResize({
    ...
    messageCallback: function(messageData) {
        if (messageData.message == 'rafflepress_loaded') {
            var el = document.querySelector(".rafflepress_iframe_loading");
...
}, '#rafflepress-<?php echo $a['id'] ?>');
}catch(error) {}
function rafflepress_resize_iframe_<?php echo $a['id'] ?>(){
    //alert('loaded');
    iFrameResize({
    log: false,
    messageCallback: function(messageData) {
